...
function handleOnBeforeRequestNonProxyWebmails(details) { // For non-proxy webmails such as outlook
	...
	if (webmail !== false) { // is from webmail
		...
		// if (webmail.name === 'ymail'){ // Yahoo's proxy server does not allow any modification to url, so will need to remove the trocker marks
		// 	var redirectUrl = details.url;
		// 	redirectUrl = redirectUrl.replace(nonSuspMark+'=1', '');
		// 	redirectUrl = redirectUrl.replace(trIgnoreMark+'=1', '');
		// 	redirectUrl = redirectUrl.replace(nonSuspMark+'1', '');
		// 	redirectUrl = redirectUrl.replace(trIgnoreMark+'1', '');
		// 	redirectUrl = redirectUrl.replace(nonSuspMark, '');
		// 	redirectUrl = redirectUrl.replace(trIgnoreMark, '');
